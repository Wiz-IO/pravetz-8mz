...
/* Stack push/pull */
#define PULL() bank_readbyte((++S) + STACK_OFFSET)
#define PUSH(value) bank_writebyte(STACK_OFFSET + (S--), (uint8)(value))
...
static mos_context cpu;
static int remaining_cycles = 0; /* so we can release timeslice */

/* memory region pointers */
//// static uint8 *ram = NULL, *stack = NULL;
static uint8 null_page[MOS_BANKSIZE];

/*
** Zero-page helper macros
*/

#define ZP_READBYTE(addr) bank_readbyte(addr)
#define ZP_WRITEBYTE(addr, val) bank_writebyte(addr, val)

static uint32 bank_readword(uint32 address)
{
   return (uint32)(bank_readbyte(address) | (bank_readbyte(address + 1) << 8));
}

static uint32 zp_readword(uint8 address) { return bank_readword(address); }

static uint8 nes_bank_readbyte(uint32 address)
{
   return cpu.mem_page[address >> MOS_BANKSHIFT][address & MOS_BANKMASK];
}

static void nes_bank_writebyte(uint32 address, uint8 value)
{
   cpu.mem_page[address >> MOS_BANKSHIFT][address & MOS_BANKMASK] = value;
}

/* read a byte of 6502 memory */
static uint8 nes_mem_readbyte(uint32 address)
{
   memread_t *mr;
...
static void nes_mem_writebyte(uint32 address, uint8 value)
{
   memwrite_t *mw;
   /* RAM */
   if (address < 0x800)
...
void nes_setup(void)
{
   bank_readbyte = nes_bank_readbyte;
   bank_writebyte = nes_bank_writebyte;
   mem_readbyte = nes_mem_readbyte;
   mem_writebyte = nes_mem_writebyte;
}

/* set the current context */
void mos_setcontext(mos_context *context)
{
   int loop;
   MOS_ASSERT(context);
   cpu = *context;
   /* set dead page for all pages not pointed at anything */
   for (loop = 0; loop < MOS_NUMBANKS; loop++)
   {
...
void mos_getcontext(mos_context *context)
{
   int loop;
   MOS_ASSERT(context);
   *context = cpu;
...
uint8 mos_getbyte(uint32 address)
{
   return bank_readbyte(address);
}

/* get number of elapsed cycles */
uint32 mos_getcycles(bool reset_flag)
{
...
#define OPCODE_END                                          \
   if (remaining_cycles <= 0)                               \
      goto end_execute;                                     \
   log_printf(mos_disasm(PC, COMBINE_FLAGS(), A, X, Y, S)); \
...
*/
int mos_execute(int timeslice_cycles)
{
...
void mos_burn(int cycles)
{
   cpu.burn_cycles += cycles;
}

/* Release our timeslice */
void mos_release(void)
{
   remaining_cycles = 0;
}
