...
PACKED_IAR struct PACKED_GNU _ring_buffer {
	...
	int            (*peek)( ring_buffer_t *rb, void *data, unsigned int data_len );
	// extract items from the head of the buffer, advancing the head
	int            (*read)( ring_buffer_t *rb, void *data, unsigned int data_len );
	// inject items at the tail of the buffer, increasing its length
	int            (*write)( ring_buffer_t *rb, void *data, unsigned int data_len );
	// like write, except take the input from another ring buffer instead of from an array
	int            (*send)( ring_buffer_t *rb, ring_buffer_t *input, unsigned int data_len );
	// advance the head of the buffer, decreasing its length
	int            (*skip)( ring_buffer_t *rb, unsigned int data_len );
	// the length of the buffer
	unsigned int       (*size)( ring_buffer_t *rb );
	// the length of the unused portion of the buffer
	unsigned int       (*available)( ring_buffer_t *rb );
